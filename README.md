...
6. **결과 정리/기록**: `core/run_opt_logging.py`와 `core/run_opt_metadata.py`가 실행 로그, 이벤트 로그, 설정 스냅샷, 결과 파일을 표준 디렉토리 구조로 정리합니다.

### 핵심 설계 포인트
- **단일 엔트리포인트**: `run_opt.py` 하나로 인터랙티브/비-인터랙티브, 큐 실행까지 지원합니다. (내부 구현은 `core/` 패키지)
